...
class ModelBase:
    ...
    def __init__(
        ...
        self.savedir = Path(f"{savedir}/{model_type}")
        ...
        self.gp = None
        if gaussian_params["use_gp"]:
            self.gp = GaussianProcess(gaussian_params)

    def run(self,rnn_params):
    
        """
        Train the models

        """
        path_to_histogram = rnn_params['data_path']
        ...
        pred_years = rnn_params["pred_years"]
        if pred_years == "None":
            pred_years = range(2019, 2020)
        elif type(pred_years) is int:
            pred_years = [pred_years]
        
        if rnn_params["times"] == "all":
            times = [32]
        else:
            times = range(10, 31, 4)

        for pred_year in pred_years:
            for run_number in range(1, rnn_params["num_runs"] + 1):
                for time in times:
                    print(
                    ...
                    results = self._run_1_year(
                        ...
                        time,
                        run_number,
                        rnn_params["train_steps"],
                        rnn_params["batch_size"],
                        rnn_params["starter_learning_rate"],
                        rnn_params["weight_decay"],
                        rnn_params["l1_weight"],
                        rnn_params["patience"],
                    )
