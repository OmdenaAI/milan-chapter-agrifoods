...
#Fruits
fruits_ohe = joblib.load('pages/fruits_ohe.pkl')
fruits_model = open('pages/fruits_rf.pkl', 'rb')
fruits_model = joblib.load(fruits_model)

def predict(input_df):
    predictions_df = fruits_model.predict(input_df)
    return predictions_df[0]


def main():
        st.subheader("Fruits & Nuts")
        ...
        total_ha = st.number_input('Total area(ha)', min_value=0, max_value=430)       
        ...
        crop = st.selectbox('Type of crop', ['peach', 'pear', 'apple', 'apricot', 'hazelnut',
        ...
        fruits_ohe = joblib.load('pages/fruits_ohe.pkl')
        fruits_ohe_dummies = fruits_ohe.transform(input_df)
        fruits_ohe_dummies_df = pd.DataFrame(data = fruits_ohe_dummies.toarray(), 
                                                #  columns = imported_sklearn_ohe.categories_
                                                columns = [c for cat in fruits_ohe.categories_ for c in cat])
        ...
        inputs = pd.concat([input_numbers, fruits_ohe_dummies_df], axis=1)
       
        if st.button("Fruts and Nuts"):
